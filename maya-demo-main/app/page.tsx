 ...
 return (
   <main className="min-h-screen bg-black flex items-center justify-center p-4">
     <div className="relative z-10 flex flex-col items-center justify-center gap-4">
       ...
       <p className="text-white animate-pulse">Initializing speech recognition...</p>
     </div>
   </main>
 )
}
   ...
   <main className="min-h-screen bg-black flex items-center justify-center p-4">
     <div className="relative z-10 flex flex-col items-center justify-center gap-8 max-w-2xl">
       <div className="text-center space-y-4">
         <h1 className="text-2xl font-bold text-white">Browser Speech Recognition Not Supported</h1>
...
const handleAfterStop = async () => {
 ...
 try {
   ...
   const res = await fetch(`${apiBase.replace(/\/+$/, '')}/conversation`, {
     ...
     body: JSON.stringify({ user_message: text, session_id: 'ca34c8a6b249412aa76085936828c1dc' }),
   })
   console.log("REPOSNE FROM THE SERVR======")
   console.log(res)
   if (!res.ok) {
       ...
       <div className="text-white/90 space-y-4">
         ...
         {!hasSpokenBefore && (
           <div className="max-w-4xl mx-auto space-y-4 text-left">
             ...
             <div>
               <p className="text-xl font-semibold uppercase tracking-wide text-white">Try:</p>
               <ul className="mt-2 space-y-2 text-xl text-white list-disc list-inside">
         ...
         {hasSpokenBefore && currentUserMessage && <span className="block mt-2 text-4xl font-semibold text-white">{currentUserMessage}</span>}
         {/* Show interim transcript while speaking */}
         {transcript && !currentUserMessage && !isSpeaking && <span className="block mt-2 text-4xl font-semibold text-white">{transcript}</span>}
